...
plot(lambda, l.lambda(lambda), ty = "l", main = "Profile log-likelihood for lambda")
```
...
l.theta <- function(theta){
  -d*log(theta*Y2 + Y1) + d2*log(theta) - (d*Y2)/(theta*Y2+Y1)*theta
}
plot(theta, l.theta(theta), ty = "l", main = "Profile log-likelihood for theta", col = "red", lty = 1)
lines(lambda, l.lambda(lambda), col = "blue", lty = 2)
legend("bottomright", legend = c("theta", "lambda"), col = c("red", "blue"), lty = 1:2)
# Does not make sense adding them to same plot I think!
```
...
loglik.prob2 <- function(lambda, theta){
  d*log(lambda)+d2*log(theta)-lambda*Y1-theta%*%lambda*Y2 
}
lambda.hat <- d1/Y1
theta.hat <- d2/d1*Y1/Y2

relative.like <- function(lambda, theta){
  loglik.prob2(lambda, theta)/loglik.prob2(lambda.hat, theta.hat)
}
grid <- expand.grid(lambda, theta)
contour(x = lambda, y = theta, z = matrix(relative.like(lambda, theta), nrow = 100))
# How can I make the grid such that z is a proper matrix!?
```
